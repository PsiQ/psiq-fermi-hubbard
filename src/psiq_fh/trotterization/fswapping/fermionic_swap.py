...
def local_fswap_op(qubit_i, qubit_j, ctrl=0):
    """Apply one nearest-neighbor fermionic swap between two adjacent wires.

        1) Apply the parity/sign operation required for fermions i.e.,a CZ
...
class FermionicSwapWithReplace(Qubrick):
    ...
    Swaps qubits `a` and `b`(with `a < b`) while
    restoring all intermediate qubits to their original order.
    """

    def __init__(self, **kwargs):
        """Initialize the fermionic swap with replace Qubrick.
    ...
    def _compute(
        ...
        Note:
            ...
            circuit drawings easier to inspect.
        """
        ...
        for a, b in zip(indices_of_first_qubits, indices_of_second_qubits):
            ...
            if a == b:
                ...
                local_fswap_op(qubits[a], qubits[b], ctrl=ctrl)
            else:
                for index in range(b, a, -1):
                    local_fswap_op(qubits[index], qubits[index - 1], ctrl=ctrl)
...
class FermionicSwapWithReplaceAVOpt(Qubrick):
    ...
    def __init__(self, use_black_box=False, **kwargs):
        """Initialize the AV optimized fermionic swap with replace Qubrick.
    ...
    def _compute(self, qubits, indices_of_first_qubits, indices_of_second_qubits, ctrl=0):
        """Perform AV optimized fermionic swaps between two sets of fermionic sites (potentially non-local).
...
def av_decomp2_fswap(n):
    """Function returns the manually calculated AV count for the fSWAP of size n using decomposition 2.

    Analytically the AV for this decomposition is ceil(3(n-1)/2) + 11.

    Args:
        n (int): size of fSWAP block, given that we are trying to fswap modes i and i+n
    Returns:
        total_av (int): number of active volume blocks
    """
    total_av = 0
    ...
    total_av += get_av_from_op(cx_op)
    return total_av


def av_decomp1_fswap(n):
    """Function returns the manually calculated AV count for the fSWAP of size n using decomposition 1.
...
def calc_crossover():
    ...
    Where n is the "size" of the fswap, i.e. when swapping fermion labelled i with fermion labelled i + n
    The crossover occurs at the value of n where ceil(3n/2) > 7.
