...
class PlaquetteTrotterStep(Qubrick):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Note:
            ...
            - Note if exptXXYY_qubrick is None then the associated rotations are implemented directly without hamming weight phasing.
        """
    ...
    def _compute(self, target_reg, data, ctrl=0):
        """Compute the time-evolution of all plaquettes grouped by the plaquette_indices.
        ...
        Args:
            ...
            data (PlaquetteTermData): Plaquette term dataclass.
        ...
        # Sign: angle is positive because the sign of kinetic term is assumed to be negative
        ...
        # Collate the site indices to pass into Fermionic swap (Separated out for testing - more circuit efficient to locaise to this arrangement).
        sites = [1, 2, 3, 4]
...
class exptXXYY(Qubrick):
    ...
    def __init__(self, batched_hamming_weight_phasing_qubrick=None, **kwargs):
        """Implement exptXXYY - see equation E13 https://arxiv.org/pdf/2012.09238.
    ...
    def _compute(
        ...
        """Computes exptX_AX_BY_AY_B in parallel over qubit lists A and B. We use the gate decomposition here versus using the
           PPR representation of the XX and YY terms.

        Args:
            site_A_qubits (Qubits): Firsts of pairs of qubits in plaquette.
            site_B_qubits (Qubits): Seconds of pairs of qubits in plaquette.
            rz_rotation_angle (float): Rotation angle - depends on the evolution time and Hamiltonian
            ctrl (int or Qubits, optional): The quantum controls that control the action of the Qubrick.
...
class exptXXYYViaPPR(Qubrick):
    ...
    def __init__(self, batched_hamming_weight_phasing_qubrick=None, use_black_box=False, **kwargs):
        """Implement exptXXYY - see equation E13 https://arxiv.org/pdf/2012.09238.
        ...
        Args:
            batched_hamming_weight_phasing_qubrick (Qubrick): Qubrick for computing the hamming weight phasing.
                If None, rotations will be simulated directly.
            use_black_box (bool): If True, uses a black box implementation with optimized AV counters.
            **kwargs: Other arguments to pass to the init.
...
class TwoModeFFFTViaControlledHad(Qubrick):
    ...
    def __init__(self, **kwargs):
        ...
        Note:
            This uses the decomposition shown in Fig. 5 of
    ...
    def _compute(self, qubits_i, qubits_j, ctrl=0):
        """Apply the two mode FFFT transform.

        Args:
            qubits_i (Qubit, list): The first qubit(s) to apply two-mode FFFT onto; can
                be a list of Qubits
            qubits_j (Qubit, list): The second qubit(s) to apply two-mode FFFT onto; can
                be a list of Qubits
        ...
        for index in range(len(qubits_i)):
            qc.z(0, ctrl | qubits_i[index] | qubits_j[index])
...
class TwoModeFFFTViaPPRs(Qubrick):
    ...
    def _compute(self, qubits_i, qubits_j, ctrl=0):
        ...
        Args:
            qubits_i (Qubit, list): The first qubit(s) to apply F onto; can
                be a list of Qubits
            qubits_j (Qubit, list): The second qubit(s) to apply F onto; can
                be a list of Qubits
            ctrl (Qubit): Control qubit
        ...
        if self.use_black_box:
            ...
            # and its dagger get cancelled in this specific case.
